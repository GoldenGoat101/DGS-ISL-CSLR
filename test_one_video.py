...
annotation_path=f'./preprocess/{dataset}/test_info.npy'
# original Annotation:
import os
import numpy as np

video_name=os.path.basename(os.path.dirname(args.video_path))
info_dict = np.load(annotation_path, allow_pickle=True).item()
for k, v in info_dict.items():
    if isinstance(v, dict) and video_name in v['folder']:
        print("Input Video Name :", video_name)
        print("Original Gloss sentence:", v['label'])
        break
...
#print("Original Sentence Glosses for the input video: ")
print(' the sentence output glosses are : {}'.format(ret_dict['recognized_sents']))
# Example 
